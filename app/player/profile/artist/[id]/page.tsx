...
export default function ArtistPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    console.log("this is not my id", params.id);
    const fetchArtistData = async () => {
      try {
        const res = await axios.get(
          `${USER_MANAGEMENT_SERVICE_URL}/artists?artistId=${params.id}`
        );
      ...
      <div className="relative z-10 p-8 pt-32">
        ...
        <div className="mb-6 flex flex-col md:flex-row items-end gap-10 md:gap-16 relative">
          <div className="relative group -mt-32 md:mt-0">
            ...
            >
              {artistData?.profile?.profileImageURL ? (
                <Image
                  src={artistData?.profile?.profileImageURL}
                  alt="Profile Photo"
                  fill
