...
export function ProfilePage() {
  ...
  const userPlaylists = [
    { name: "My Favorites", songCount: 47, image: "/music-playlist-cover.png" },
    { name: "Chill Vibes", songCount: 32, image: "/chill-music-playlist.png" },
    { name: "Workout Mix", songCount: 28, image: "/workout-music-playlist.png" },
    { name: "Late Night", songCount: 19, image: "/late-night-music-playlist.png" },
  ]
  ...
  return (
    <div className="h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-orange-900/20 text-white overflow-y-auto custom-scrollbar">
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute bottom-20 right-20 w-80 h-80 bg-orange-400/5 rounded-full blur-3xl animate-pulse delay-1000" />
        <div className="absolute top-1/2 left-1/2 w-64 h-64 bg-orange-600/8 rounded-full blur-3xl animate-pulse delay-500" />
      </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {userPlaylists.map((playlist, index) => (
              <div key={index} className="group cursor-pointer">
                <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-4 hover:bg-white/20 transition-all duration-300 hover:scale-105 hover:shadow-2xl">
                  <div className="relative mb-4">
                    ...
                    />
                    <div className="absolute inset-0 bg-gradient-to-br from-orange-500/20 to-orange-600/20 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
                    <Button
                      size="icon"
                      className="absolute bottom-2 right-2 bg-orange-500 hover:bg-orange-600 text-white rounded-full w-12 h-12 opacity-0 group-hover:opacity-100 transform translate-y-2 group-hover:translate-y-0 transition-all duration-300 shadow-lg"
                    >
                      <Play className="w-5 h-5" />
                    </Button>
                  </div>
                  <h3 className="font-semibold text-white mb-1">{playlist.name}</h3>
                  <p className="text-sm text-white/60">{playlist.songCount} songs</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="flex gap-6 overflow-x-auto pb-4 custom-scrollbar">
            {topArtists.map((artist, index) => (
              <div key={index} className="flex-shrink-0 group cursor-pointer">
                <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-4 hover:bg-white/20 transition-all duration-300 hover:scale-105 hover:shadow-2xl w-48">
                  <div className="relative mb-4">
                    <img
                    ...
                    />
                    <div className="absolute inset-0 bg-gradient-to-br from-orange-500/20 to-orange-600/20 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
                  </div>
                  <h3 className="font-semibold text-white text-center">{artist.name}</h3>
        ...
        <div>
          ...
          <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-6">
            {topTracks.map((track, index) => (
              ...
              >
                <span className="text-white/60 font-medium w-6 text-center group-hover:hidden">{index + 1}</span>
                <Button
                  size="icon"
                  variant="ghost"
                  className="w-6 h-6 hidden group-hover:flex text-white hover:text-orange-400"
                >
                ...
                <p className="text-sm text-white/60 hidden md:block">{track.album}</p>
                <Button
                  size="icon"
                  variant="ghost"
                  className="text-white/60 hover:text-orange-400 opacity-0 group-hover:opacity-100 transition-all duration-200"
                >
                  <Heart className="w-4 h-4" />
                </Button>
                <span className="text-sm text-white/60 w-12 text-right">{track.duration}</span>
              </div>
            ))}
          </div>
        </div>
